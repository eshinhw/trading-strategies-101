...
// unaffected — this only bypasses the unlock check. Checked against the
// email in the session JWT (see attachUser) rather than a DB lookup, since
// this now runs on every /modules request.
const FULL_ACCESS_EMAILS = new Set(["examtest@example.com"]);

function hasFullContentAccess(userEmail?: string): boolean {
  return userEmail ? FULL_ACCESS_EMAILS.has(userEmail) : false;
}
...
router.get("/modules", async (req, res) => {
  ...
  const fullAccess = hasFullContentAccess(req.userEmail);
